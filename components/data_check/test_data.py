...
def test_row_count(data: pd.DataFrame) -> None:
    ...
    Udacity expects the cleaned dataset to not be empty and not absurdly large.
    """
...
def test_price_range(data: pd.DataFrame, min_price: float, max_price: float) -> None:
    """
    Ensure all prices fall within the configured min/max range.
    These values come from the pytest command-line options.
    """
